...
function getVideoByText() {
	mui.showLoading("正在播放，请稍等");
	if(ybResult==""){
		ybResult="获取内容失败请稍后在试。"
	}
	$.ajax({
		type : "GET",
		url :'https://chuhewentian.com:15014/shandongt/speechRecognition.do?createVideoByText&content='+ybCounty+ybResult,
		dataType : 'json',
...
function showHourWeather(res) {
	...
	if (res.length > 0) {
        ...
        for (let i = 0; i < hourList.length; i++) {
            ...
            if (hours != '') {
              hours = hours.substring(8, 10)+":"+hours.substring(10, 12);
            }
		    ...
		    var li = $('<li><img src="'+imgUrl+'">'
						+'<div class="weather-fx"><p>'+getHourFengXing(hourList[i].dd_level)+'</p></div>'
						+'<div class="weather-fs"><p>'+getHourWindLevel(hourList[i].ff_level)+'</p></div></li>'); 
		    $(".houryb").append(li);
	...
	document.getElementById('yb3').style.width = "calc((50px *"+tempArr.length+"))";
...
function showWarning(data) {
	...
	if(data.length!=0){
		...
		for(var i=0;i<data.length;i++){
			...
			divObj.data("data", data[i]);
			$(".swiper-wrapper").append(divObj);
			
		}
		mui(".swiper-wrapper").on('tap','.swiper-slide',function() {
			
			var datavalue = $(this).data("data");
			localStorage.setItem('iframeData',
			JSON.stringify(datavalue));
			var searchUrl = encodeURI("../warning/warningInfo.html?dataArray="+ datavalue);
			parent.location.href = searchUrl;
		});
		
		mui(".swiper-wrapper").on('click','.swiper-slide',function() {
			var datavalue = $(this).data("data");
			localStorage.setItem('iframeData',
			JSON.stringify(datavalue));
			var searchUrl = encodeURI("../warning/warningInfo.html?dataArray="+ datavalue);
...
function showLive(data) {
	...
	ybResult="当前温度"+data.tem+"度"+"湿度百分之"+data.rhu+"降水量"+data.pre_1h+"毫米"+getFengXing(data.win_d_avg_10mi)+getLiveWindLevel(data.win_s_avg_10mi) + "级"
}
// 两天预报、七天预报
