...
var WindyStyle = {
		VELOCITY_SCALE : 0.02 * (Math.pow(window.devicePixelRatio, 1 / 3) || 1), // 线长度
		PARTICLE_LINE_WIDTH : 1, // 线的宽度
		PARTICLE_MULTIPLIER : 1 / 100, // 密度
		FRAME_RATE : 15,
		FRAME_TIME : 800 / 15, // 速度
		color : "rgb(255,255, 255)", // 线的颜色
		
		
		setLineType : function(obj){
			if(obj == null || obj == ""){
				return;
			}
			if(obj.VELOCITY_SCALE != null) WindyStyle.VELOCITY_SCALE = obj.VELOCITY_SCALE;
			if(obj.PARTICLE_LINE_WIDTH != null) WindyStyle.PARTICLE_LINE_WIDTH = obj.PARTICLE_LINE_WIDTH;
			if(obj.PARTICLE_MULTIPLIER != null) WindyStyle.PARTICLE_MULTIPLIER = obj.PARTICLE_MULTIPLIER;
			if(obj.FRAME_TIME != null) WindyStyle.FRAME_TIME = obj.FRAME_TIME;
			if(obj.FRAME_RATE != null) WindyStyle.FRAME_RATE = obj.FRAME_RATE;
			if(obj.color != null) WindyStyle.color = obj.color;
//			if(obj.VELOCITY_SCALE != null) VELOCITY_SCALE = obj.VELOCITY_SCALE;
//			if(obj.VELOCITY_SCALE != null) VELOCITY_SCALE = obj.VELOCITY_SCALE;
//			var VELOCITY_SCALE = 0.02 * (Math.pow(window.devicePixelRatio, 1 / 3) || 1); // 线长度
...
//			var PARTICLE_LINE_WIDTH = 1; // 线的宽度
//			var PARTICLE_MULTIPLIER = 1 / 100; // 密度
//			var PARTICLE_REDUCTION = Math.pow(window.devicePixelRatio, 1 / 3) || 1.6; // multiply particle count for mobiles by this amount
//			var FRAME_RATE = 15,
//			    FRAME_TIME = 800 / FRAME_RATE; // 运行速度
		}
}
...
var Windy = function Windy(params) {
 	/**参数设置*/
	var VELOCITY_SCALE = WindyStyle.VELOCITY_SCALE; // 线长度
	var MIN_TEMPERATURE_K = 261.15; // step size of particle intensity color scale
	var MAX_TEMPERATURE_K = 317.15; // wind velocity at which particle intensity is maximum (m/s)
	var MAX_PARTICLE_AGE = 90; // max number of frames a particle is drawn before regeneration
	var PARTICLE_LINE_WIDTH = WindyStyle.PARTICLE_LINE_WIDTH; // 线的宽度
	var PARTICLE_MULTIPLIER = WindyStyle.PARTICLE_MULTIPLIER; // 密度
	var PARTICLE_REDUCTION = Math.pow(window.devicePixelRatio, 1 / 3) || 1.6; // multiply particle count for mobiles by this amount
	var FRAME_RATE = WindyStyle.FRAME_RATE,
	    FRAME_TIME = WindyStyle.FRAME_TIME; // 运行速度
	var color = WindyStyle.color; // 颜色
...
//		map.off('mousemove', this._onMouseMove, this);
	...
	_onMouseMove: function _onMouseMove(e) {
		...
		if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) ) {
			...
			htmlOut = "<span style='position: absolute;width: 1000px;top: -55px;font-size:18px'><strong style='font-size:18px'>Wind Direction: </strong>" + self.vectorToDegrees(gridValue[0], vMs) + "°" + ", <strong  style='font-size:18px'>Wind Speed: </strong>" + self.vectorToSpeed(gridValue[0], vMs).toFixed(1) + "m/s";
		} else {
		//position: absolute;
		//width: 1000px;
		//top: -100px;

			htmlOut = "<span style='position: absolute;width: 500px;top: -55px;font-size:18px'>no wind data</span>";
		}
...
(function (root, factory) {
	...
	var WindJSLeaflet = {
		...
		init: function init(options) { 
			...
			WindJSLeaflet._checkWind(options).then(function () { 
				...
				WindJSLeaflet._localData=options.localData;
		...
		// 开关控制
		windSwitch : function windSwitch(type){
			if(type == "open"){
				if(!WindJSLeaflet._WindType){
					WindJSLeaflet._canvasLayer.onAdd(WindJSLeaflet._map); // 把风场叠加到地图上
					WindJSLeaflet._WindType = true;
				}
			}else{
				if(WindJSLeaflet._WindType){
					WindJSLeaflet._canvasLayer.onRemove(WindJSLeaflet._map); // 清楚地图上的叠加层
					WindJSLeaflet._destroyWind(); // 清楚对叠加曾的控制，避免拖动地图时重新生成
					WindJSLeaflet._WindType = false;
					WindJSLeaflet._data = null;
				}
			}
		},
   ...
   */
		...
		_loadLocalData: function _loadLocalData(url) {
			WindJSLeaflet._localData = url == null || url == "" ? WindJSLeaflet._localData : url;
			$.getJSON(WindJSLeaflet._localData, function (data) { 
		/*												  
				for(var i=0;i<data.length;i++){
	
					data[i].header.nx=73*0.25;
					data[i].header.ny=109*0.25;
					data[i].header.dx=1;
					data[i].header.dy=1;

 					data[i].header.la1=35.000004;
					data[i].header.la2=8.000004;
					data[i].header.lo1=92;
					data[i].header.lo2=108.64;

				}
				*/
				if(WindJSLeaflet._data != null){
					WindJSLeaflet._windy.stop();
				}
